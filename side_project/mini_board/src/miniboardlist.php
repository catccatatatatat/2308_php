...
try {
	...
	$max_page_num = ceil($board_cnt / $list_cnt); // 최대 페이지 수
	...
	if(!$result) {
		...
		throw new Exception("DB Error : SELECT board"); // 강제 예외발생 : SELECT board
	}
...
<body>
	...
	<main>
		...
		<section>
			<a class="page-btn" href="/mini_board/src/miniboardlist.php/?page=<?php echo $prev_page_num ?>">이전</a>
			<?php 
				for($i = 1; $i <= $max_page_num; $i++) {
					// 삼항연산자 : 조건 ? 참일때처리 : 거짓일때처리
					$str = (int)$page_num === $i ? "bk-a" : "";
			?>
					<a class="page-btn <?php echo $str ?>" href="/mini_board/src/miniboardlist.php/?page=<?php echo $i; ?>"><?php echo $i; ?></a>
			<?php
				}
			?>
			<a class="page-btn" href="/mini_board/src/miniboardlist.php/?page=<?php echo $next_page_num ?>">다음</a>
		</section>
	</main>
</body>
</html>